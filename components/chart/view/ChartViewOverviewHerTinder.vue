...
export default {
  ...
  methods: {
    ...
    drawViz() {
      ...
      const orientationChart = new dc.PieChart(
        `#sexualOrientations-chart-${this.graphId}`
      )
      ...
      likesChart.title(
        'Passes',
        d => `${dateFormatter(d.key)}: ${d.value} passes`
      )
      likesChart.stack(matchesGroup, 'Matches')
      likesChart.title(
        'Matches',
        d => `${dateFormatter(d.key)}: ${d.value} matches`
      )
      ...
      messagesChart.title(
        'Messages Received',
        d => `${dateFormatter(d.key)}: ${d.value} messages received`
      )
      messagesChart.title(
        'Messages Sent',
        d => `${dateFormatter(d.key)}: ${d.value} messages sent`
      )
