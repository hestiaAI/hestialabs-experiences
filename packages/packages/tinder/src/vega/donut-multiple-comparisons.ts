...
export default (labels: string[]) => {
  const labelsMap = Object.fromEntries(
    labels.map((label, index) => [options[index], label])
  )
  return {
    ...
    title: {
      text: {
        signal: "versusLabels[versus[0]] + ' vs ' + versusLabels[versus[1]]"
      },
      subtitle: {
        signal:
          '\'between \' + timeFormat(extent[0], "%d/%m/%Y") + \' and \' + timeFormat(extent[1], "%d/%m/%Y")'
      },
      dy: {
        value: -24
      },
